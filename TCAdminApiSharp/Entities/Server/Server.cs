...
public class Server : ObjectBase, IObjectBaseCrud<Server>
{
    ...
    public async Task<bool> Update(Action<Server> action)
    {
        ...
        var request = TcaClient.ServersController.GenerateDefaultRequest(ServerId.ToString());
        request.Method = HttpMethod.Put;
        request.Content = new StringContent(putJson, Encoding.UTF8, Constants.JsonContentType);
        return (await TcaClient.ServersController.ExecuteBaseResponseRequest(request)).Success;
    }
